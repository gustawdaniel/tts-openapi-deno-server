...
export class S3Adapter extends StorageAdapter {
  ...
  constructor() {
    ...
    if (env.S3_ACCESS_KEY_ID && !Deno.env.get("S3_ACCESS_KEY_ID")) {
      Deno.env.set("S3_ACCESS_KEY_ID", env.S3_ACCESS_KEY_ID);
    }
    if (env.S3_SECRET_ACCESS_KEY && !Deno.env.get("S3_SECRET_ACCESS_KEY")) {
      Deno.env.set("S3_SECRET_ACCESS_KEY", env.S3_SECRET_ACCESS_KEY);
    }
    if (env.S3_REGION && !Deno.env.get("S3_REGION")) {
      Deno.env.set("S3_REGION", env.S3_REGION);
    }
    if (env.S3_BUCKET_NAME && !Deno.env.get("S3_BUCKET_NAME")) {
      Deno.env.set("S3_BUCKET_NAME", env.S3_BUCKET_NAME);
    }
    if (env.S3_DIR && !Deno.env.get("S3_DIR")) {
      Deno.env.set("S3_DIR", env.S3_DIR);
    }
    if (env.S3_ENDPOINT_URL && !Deno.env.get("S3_ENDPOINT_URL")) {
      Deno.env.set(
        "S3_ENDPOINT_URL",
        env.S3_ENDPOINT_URL.startsWith("https://")
          ? env.S3_ENDPOINT_URL
          : `https://${env.S3_ENDPOINT_URL}`,
      );
    }
    ...
    if (!Deno.env.get("S3_ENDPOINT_URL")!.startsWith("https://")) {
      Deno.env.set(
        "S3_ENDPOINT_URL",
        `https://${Deno.env.get("S3_ENDPOINT_URL")}`,
      );
    }
    ...
    this.base = `https://${Deno.env.get("S3_BUCKET_NAME")}.${
      Deno.env.get("S3_ENDPOINT_URL")!.replace(/^https?:\/\//, "")
    }`;
  }
  ...
  async get(appKey: string): Promise<Response | undefined> {
    ...
    if (exists) {
      return new Response(null, {
        status: 302,
        headers: { "Location": `${this.base}/${key}` },
      });
    }
  }
